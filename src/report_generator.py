...
class ReportGenerator:
    ...
    def generate_report(self, analysis_results: List[Dict[str, Any]],
        ...
        try:
            ...
            for result in analysis_results:
                if result.get("has_violation", False):
                    violations_list.append({
                        ...
                        "applied_rules": result.get("applied_rules", [])[:3],  # Топ-3 правила
                        "confidence": result.get("confidence", 0.0)
                    })

            total_violations = len(violations_list)
            correct_chunks = total_chunks - total_violations
            compliance_rate = (correct_chunks / total_chunks * 100) if total_chunks > 0 else 0.0
            ...
            report = {
                ...
                "details": analysis_results  # Полные данные по каждому чанку
            }

            logger.info(f"Отчёт сгенерирован: {total_violations} нарушений, статус={report['summary']['status']}")
            return report
        ...
        if violations:
            ...
            for i, v in enumerate(violations, 1):
                ...
                for viol in v.get('violations', []):
                    lines.append(f"    ❌ Нарушение: {viol.get('description', 'Нет описания')}")
    ...
    def save_report_json(self, report: Dict[str, Any], output_path: str):
        """
